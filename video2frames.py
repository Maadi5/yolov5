...
def main(args):
    ...
    while frameId < frameCount:
        ...
        char_l = len(str(framename))
        req_zeros = max_l - char_l
        if req_zeros>0:
            lst = np.zeros(req_zeros).astype(np.uint8).astype(np.str)
            join_st = ''.join(lst.tolist())
            fname = "frame_" + join_st + str(framename) + ".jpg"
        else:
            fname = "frame_" + str(framename) + ".jpg"
